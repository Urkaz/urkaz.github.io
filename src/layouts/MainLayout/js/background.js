...
function Update(e) {
  ...
  for (i = 0; i < numPoints; i++) {
    ...
    if (direction < 0.2) pointsVelocities[i][0] += speed;
    else if (direction < 0.4) pointsVelocities[i][0] -= speed;
    else if (direction < 0.6) pointsVelocities[i][1] += speed;
    else if (direction < 0.8) pointsVelocities[i][1] -= speed;
    ...
    //Cap Max Velocity
    if (pointsVelocities[i][0] >= maxVelocity)
      pointsVelocities[i][0] = maxVelocity;
    if (pointsVelocities[i][0] <= -maxVelocity)
      pointsVelocities[i][0] = -maxVelocity;
    if (pointsVelocities[i][1] >= maxVelocity)
      pointsVelocities[i][1] = maxVelocity;
    if (pointsVelocities[i][1] <= -maxVelocity)
      pointsVelocities[i][1] = -maxVelocity;
  }
  ...
  if (allowRender) {
    for (i = 0; i < numPoints + 1; i++) {
      ...
      for (var j = i; j < numPoints + 1; j++) {
        var distanceBetweenPoints = distanceCheck(
          pointsArray[i],
          pointsArray[j]
        );
...
function Resize() {
  ...
  if (
    Math.abs(newWidth - oldWidth) > resizeRespawnThreshold ||
...
function distanceCheck(pointOne, pointTwo) {
  var dist_points =
    (pointOne[0] - pointTwo[0]) * (pointOne[0] - pointTwo[0]) +
